...
let ObjectControls = function (camera, domElement) {
    ...
    this.attach = function (object) {
        ...
        if (object instanceof THREE.Mesh) {//instanceof作用是判断其左边对象是否为其右边类的实例，返回boolean类型的数据
            this.objects.push(object);//push() 方法可向数组的末尾添加一个或多个元素，并返回新的长度。
        }
        else {
    ...
    this.detach = function (object) {
        ...
        this.objects.splice(item, 1);//splice() 方法向/从数组中添加/删除项目，然后返回被删除的项目。
    ...
    this.container.addEventListener('touchstart', onContainerMouseDown, false) ;
    this.container.addEventListener( 'touchend', getTouchPos, false ) ;
