...
(function (exports) {
	...
	AnyRTC.prototype.createStream = function (DRender) {
		...
		switch (that.liveModel) {
			...
			case 'RTCP_Videos_HD':
				...
				VideoOptions = {
					"video": that.audioLive ? false : { 'mandatory': { 'maxWidth': 1280, 'maxHeight': 720, 'minWidth': 640, 'minHeight': 480, 'maxFrameRate': 30}},
					"audio": true
				};
				break;
			...
			case 'RTCP_Videos_QHD':
				...
				VideoOptions = {
					"video": that.audioLive ? false : { 'mandatory': { 'maxWidth': 960, 'maxHeight': 540, 'minWidth': 640, 'minHeight': 480, 'maxFrameRate': 30}},
					"audio": true
				};
				break;
			case 'RTMPC_Video_SD':
			...
			case 'RTCP_Videos_SD':
				...
				VideoOptions = {
					"video": that.audioLive ? false : { 'mandatory': { 'maxWidth': 640, 'maxHeight': 480, 'minWidth': 640, 'minHeight': 288, 'maxFrameRate': 30}},
					"audio": true
				};
				break;
			case 'RTCP_Videos_myVideo':
                ...
                VideoOptions = {
                    /*"video": that.audioLive ? false : { 'mandatory': { 'maxWidth': 660, 'maxHeight': 370, 'minWidth': 660, 'minHeight': 370, 'maxFrameRate': 30}},*/
                    "video" : {
                    	optional : [{
                    		sourceId : that.videoId
						}]
					},
                    "audio": false
                };
		...
		if (!navigator.mediaDevices) {
			...
			navigator.mediaDevices.getUserMedia(VideoOptions).then(function (stream) {
				console.log("我得到的消息为" , VideoOptions);
	...
	AnyRTC.prototype.attachStream = function (stream, DRender) {
		var element = DRender;
		element.muted = true;//设置该属性后，规定视频的音频输出应该为静音
